...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_image'])) {
    ...
    // if everything is ok, try to upload file
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
            ...
            $sql = "UPDATE rooms SET image='$target_file' WHERE id='$room_id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_title'])) {
    ...
    $sql = "UPDATE rooms SET title='$title' WHERE id='$room_id'";
    if ($conn->query($sql) === TRUE) {
        $update_success = true;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_description'])) {
    ...
    $sql = "UPDATE rooms SET description='$description' WHERE id='$room_id'";
    if ($conn->query($sql) === TRUE) {
        $update_success = true;
...
<body>
    <div class="admin-panel">
        <a href="layaleisureadimpanel.php" class="btn-back">Back to Dashboard</a>
        <h2>Manage Rooms</h2>
