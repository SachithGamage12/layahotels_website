...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['booking_status'])) {
    foreach ($_POST['booking_status'] as $id => $status) {
        $sql = "UPDATE beachbookings SET booking_status = ? WHERE id = ?";
...
if (isset($_POST['delete_all'])) {
    ...
    $delete_sql = "DELETE FROM beachbookings WHERE 1";
...
function archiveBookings() {
    ...
    $select_sql = "SELECT * FROM beachbookings";
    $result = $conn->query($select_sql);

    if ($result->num_rows > 0) {
        // Prepare archive insert statement
        $archive_insert_sql = "INSERT INTO beachbookings_archive (first_name, last_name, email_address, mobile_number, selected_rooms_html, checkin_date, checkout_date, booking_status) VALUES ";
...
$conn->close();
header("Location: layabeachadminpanel.php");
exit();
?>
