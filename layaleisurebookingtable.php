...
if (isset($_POST['delete_all'])) {
    ...
    $delete_sql = "TRUNCATE TABLE leisurebookings_archive";
    if (mysqli_query($conn, $delete_sql)) {
        // Reset the ID counter to start from 1
        $reset_sql = "ALTER TABLE leisurebookings_archive AUTO_INCREMENT = 1";
        mysqli_query($conn, $reset_sql);
        
        // Redirect to the same page to avoid resubmission
        header("Location: layaleisurebookingtable.php");
        exit;
    } else {
...
$sql = "SELECT * FROM leisurebookings_archive WHERE 1";
if (!empty($search)) {
...
$total_records_query = "SELECT COUNT(*) as total_records FROM leisurebookings_archive WHERE 1";
if (!empty($search)) {
...
<head>
    ...
    <title>Admin Panel - Leisure Bookings</title>
...
<body>
    <div class="container">
        ...
        <a href="layaleisureadimpanel.php" class="btn btn-secondary mb-3">Back to Admin Panel</a>
        
        <!-- Search form -->
        <form action="layaleisurebookingtable.php" method="GET" class="form-inline mb-4">
        ...
        <!-- Delete All button -->
        <form action="layaleisurebookingtable.php" method="POST" class="mb-4">
