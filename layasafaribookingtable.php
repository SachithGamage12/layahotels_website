...
if (isset($_POST['delete_all'])) {
    ...
    $delete_sql = "TRUNCATE TABLE safaribooking_archive";
    if (mysqli_query($conn, $delete_sql)) {
        // Reset the ID counter to start from 1
        $reset_sql = "ALTER TABLE safaribooking_archive AUTO_INCREMENT = 1";
        mysqli_query($conn, $reset_sql);
        
        // Redirect to the same page to avoid resubmission
        header("Location: layasafaribookingtable.php");
        exit;
    } else {
...
$sql = "SELECT * FROM safaribooking_archive WHERE 1";
if (!empty($search)) {
...
$total_records_query = "SELECT COUNT(*) as total_records FROM safaribooking_archive WHERE 1";
if (!empty($search)) {
...
<head>
    ...
    <title>Admin Panel - Safari Bookings</title>
...
<body>
    <div class="container">
        ...
        <a href="layasafariadminpanel.php" class="btn btn-secondary mb-3">Back to Admin Panel</a>
        
        <!-- Search form -->
        <form action="layasafaribookingtable.php" method="GET" class="form-inline mb-4">
        ...
        <!-- Delete All button -->
        <form action="layasafaribookingtable.php" method="POST" class="mb-4">
